...
def load_snpedia_records(file_path):
    ...
    with open(file_path, newline="", encoding="utf-8") as fp:
        ...
        for row in reader:
            ...
            summary = (
                row.get("summary") or row.get("Summary") or row.get("description") or ""
            ).strip()
...
def synthesize_snpedia_with_llm(snpedia_matches, snp_dict=None):
    ...
    for snp in balanced_snps:
        ...
        snp_text += (
            f"- **{rsid}** (Genotype: {genotype}, Magnitude: {mag}, Repute: {repute}): {summary}\n"
        )
...
def fetch_snpedia_batch(snp_genotype_pairs):
    ...
    for rsid, genotype in snp_genotype_pairs:
        try:
            ...
            if content_genotype:
                mag_match = re.search(
                    r"magnitude\s*=\s*([-+]?[0-9]+(?:\.[0-9]+)?)", content_genotype, re.IGNORECASE
                )
            ...
            if not mag_match and content_general:
                mag_match = re.search(
                    r"magnitude\s*=\s*([-+]?[0-9]+(?:\.[0-9]+)?)", content_general, re.IGNORECASE
                )
...
class SNPAnalyzerApp(tk.Tk):
    ...
    def _configure_styles(self):
        ...
        style.map("Accent.TButton", background=[("active", "#3a7bc8"), ("pressed", "#2e5f9e")])
        ...
        style.map("Secondary.TButton", background=[("active", "#7f8c8d"), ("pressed", "#6c7a7b")])
        ...
        style.map("Modern.Treeview.Heading", background=[("active", "#3a7bc8")])
        style.map(
            "Modern.Treeview",
    ...
    def _build_ui(self):
        ...
        ttk.Button(
            file_frame, text="📂 Browse", command=self.open_file, style="Secondary.TButton"
        ).grid(row=0, column=2, padx=5)
        ttk.Button(
            file_frame, text="🔬 Analyze", command=self.analyze_file, style="Accent.TButton"
        ).grid(row=0, column=3, padx=5)
        ttk.Button(
            file_frame,
            text="📡 Fetch SNPedia",
            command=self.fetch_snpedia_api,
        ...
        summary_frame = ttk.Labelframe(
            content_container, text="📊 Genomic Summary", padding=15, style="Card.TLabelframe"
        )
        ...
        self.summary_text.tag_configure(
            "title", font=("Helvetica", 11, "bold"), foreground="#2c3e50"
        )
        self.summary_text.tag_configure(
            "value", font=("Monaco", 10, "bold"), foreground=self.accent_color
        )
        self.summary_text.tag_configure(
            "section", font=("Helvetica", 10, "bold"), foreground="#e74c3c"
        )

        # Matches card with notebook tabs
        match_frame = ttk.Labelframe(
            content_container, text="🔍 Match Results", padding=15, style="Card.TLabelframe"
        )
        ...
        self.fmf_table = ttk.Treeview(
            fmf_tab, columns=fmf_columns, show="headings", style="Modern.Treeview"
        )
        ...
        tk.Label(snpedia_toolbar, text="Threshold:", bg=self.bg_color, fg="#34495e").pack(
            side=tk.LEFT
        )
        ...
        tk.Label(snpedia_toolbar, text="Sample size:", bg=self.bg_color, fg="#34495e").pack(
            side=tk.LEFT
        )
        ...
        tk.Label(snpedia_toolbar, text="Top N (0=All):", bg=self.bg_color, fg="#34495e").pack(
            side=tk.LEFT
        )
        ...
        if HAS_OPENAI and os.environ.get("OPENAI_API_KEY"):
            ttk.Button(
                snpedia_toolbar,
                text="🤖 Synthesize with AI",
                command=self._synthesize_snpedia_results,
                style="Secondary.TButton",
            ).pack(side=tk.LEFT, padx=5)

        sp_columns = ("chrom", "rsid", "genotype", "magnitude", "repute", "summary")
        self.snpedia_table = ttk.Treeview(
            snpedia_tab, columns=sp_columns, show="headings", style="Modern.Treeview"
        )
    ...
    def fetch_snpedia_api(self):
        if not self.current_snps:
            messagebox.showinfo(
                "Load genome first",
                "Please load and analyze a 23andMe file before fetching SNPedia.",
            )
            return

        if self.fetch_thread and self.fetch_thread.is_alive():
            messagebox.showinfo(
                "Fetch already running", "Please wait for the current SNPedia fetch to finish."
            )
            return
        ...
        if use_fmf_only:
            ...
            if additional_needed > 0 and remaining:
                ...
                additional = [remaining[i] for i in range(0, len(remaining), step)][
                    :additional_needed
                ]
            else:
                additional = []
        ...
        self.status.config(
            text="📡 Fetching {} SNPedia entries... this may take a bit.".format(
                len(snp_genotype_pairs)
            )
        )
        ...
        self.fetch_thread = threading.Thread(
            target=self._snpedia_fetch_worker, args=(snp_genotype_pairs,), daemon=True
        )
    ...
    def analyze_file(self):
        ...
        status_bits = [
            "✨ Successfully loaded {:,} SNPs from {}".format(
                summary["total_snps"], os.path.basename(file_path)
            )
        ]
        status_bits.append("FMF matches: {}".format(len(fmf_matches)))
        if self.snpedia_records:
            status_bits.append(
                "SNPedia matches: {} (sorted by magnitude)".format(len(snpedia_matches))
            )
        else:
    ...
    def _show_synthesis_result(self, synthesis):
        ...
        text_widget.tag_configure(
            "link", foreground="#0066cc", underline=True, font=("Helvetica", 11, "underline")
        )
        text_widget.tag_configure("code", font=("Courier", 10), background="#f0f0f0")
        text_widget.tag_configure(
            "heading", font=("Helvetica", 13, "bold"), foreground=self.accent_color
        )
        ...
        ttk.Button(
            btn_frame, text="Close", command=result_window.destroy, style="Secondary.TButton"
        ).pack(side=tk.RIGHT)
            ...
            if content[i] == "[" and "](" in content[i:]:
                ...
                if close_bracket != -1 and open_paren != -1 and close_paren != -1:
                    ...
                    text_widget.tag_bind(
                        "link", "<Motion>", lambda e: text_widget.config(cursor="hand2")
                    )
